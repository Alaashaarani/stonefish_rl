...
class dsEnv(EnvStonefishRL):
    def __init__(self, ip="tcp://localhost:5555", search_time=30):
        ...
        self.target_threshold = 0.5 # Acceptable distance between the object and the gripper
        ...
        n_total_obs = 3+6+6 # ds pose (constant Yaw, 0) , AUV pose, AUV velocities and angular velocities. 
    ...
    def build_reset_command(self):
        ...
        #     5.0                                # z --> Pool height
        # ]
        # ds_rot = [0.0, 0.0, 0.0]  # Rotation of the "ds" robot
        ds_pos = [
            0, # x --> Pool length
            0, # y --> Pool width
            5.0                                # z --> Pool height
    ...
    def dist_target_goal(self):
        ...
        dist = np.linalg.norm(np.array(vec_xyz_AUV) - np.array(vec_xyz_ds))
        return dist
    ...
    def calculate_reward(self):
        ...
        """
        reward = 0
        actual_dist = self.dist_target_goal()
        if(self.target_threshold < actual_dist):
